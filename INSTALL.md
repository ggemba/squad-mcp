...
4. **Verify the install.** In a fresh prompt:
   ...
   - Ask Claude to call the `list_agents` tool from the `squad` MCP server. It should return 9 agents (`product-owner`, `tech-lead-planner`, `tech-lead-consolidator`, `architect`, `dba`, `developer`, `reviewer`, `security`, `qa`).
...
```
record reject security "missing CSRF on POST /api/refund"
...
-> Forces security as the primary specialist
```
...
- [ ] `list_agents` tool returns 9 agents (names: `product-owner`, `tech-lead-planner`, `tech-lead-consolidator`, `architect`, `dba`, `developer`, `reviewer`, `security`, `qa`).
...
- [ ] Resources `agent://architect` and `severity://_severity-and-ownership` are readable.
