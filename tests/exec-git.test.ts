...
function createFakeChild(opts: FakeChildOpts) {
  const child = new EventEmitter() as EventEmitter & { stdout: Readable; stderr: Readable; kill: ReturnType<typeof vi.fn>; killed: boolean };
...
describe('runGit error paths', () => {
  it('rejects non-allowlisted subcommand', async () => {
    ...
    await expect(runGit('commit', [], dir)).rejects.toMatchObject({ code: 'GIT_EXEC_DENIED' });
  });

  it('rejects forbidden flag like -c', async () => {
    const dir = await tmpRepoDir();
    await expect(runGit('diff', ['-c', 'foo=bar'], dir)).rejects.toMatchObject({ code: 'GIT_EXEC_DENIED' });
  });

  it('rejects --upload-pack arg', async () => {
    const dir = await tmpRepoDir();
    await expect(runGit('diff', ['--upload-pack=evil'], dir)).rejects.toMatchObject({ code: 'GIT_EXEC_DENIED' });
  });

  it('rejects relative cwd', async () => {
    await expect(runGit('diff', [], 'relative/path')).rejects.toMatchObject({ code: 'GIT_EXEC_DENIED' });
  });

  it('rejects cwd without .git', async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'squad-nogit-'));
    await expect(runGit('diff', [], dir)).rejects.toMatchObject({ code: 'GIT_NOT_A_REPO' });
  });

  it('honors timeout via injected spawnFn', async () => {
    const dir = await tmpRepoDir();
    const fakeSpawn = vi.fn(() => createFakeChild({ delayMs: 5_000 })) as never;
    await expect(runGit('diff', [], dir, { spawnFn: fakeSpawn, timeoutMs: 50 })).rejects.toMatchObject({
  ...
  it('enforces stdout cap mid-stream', async () => {
    ...
    await expect(runGit('diff', [], dir, { spawnFn: fakeSpawn, maxStdout: 1024 })).rejects.toMatchObject({
  ...
  it('returns stdout/stderr/code on normal exit via injected spawnFn', async () => {
    const dir = await tmpRepoDir();
    const fakeSpawn = vi.fn(() => createFakeChild({ stdoutChunks: [Buffer.from('A\tfile1.ts\n')], exitCode: 0 })) as never;
  ...
  it('translates ENOENT into GIT_NOT_FOUND', async () => {
    ...
    await expect(runGit('diff', [], dir, { spawnFn: fakeSpawn })).rejects.toMatchObject({ code: 'GIT_NOT_FOUND' });
  });
});
