...
describe("agents content guard rails", () => {
  it("debugger persona carries the read-only + no-writes boundary strings", async () => {
    const file = path.join(AGENTS_DIR, "debugger.md");
    const body = await fs.readFile(file, "utf8");
    expect(body.toLowerCase()).toContain("read-only");
    expect(body.toLowerCase()).toContain("no writes");
  });

  it("debugger persona carries the untrusted-input clause", async () => {
    const file = path.join(AGENTS_DIR, "debugger.md");
    const body = await fs.readFile(file, "utf8");
  ...
  // (Removed) debugger pin to haiku — moved to sonnet under the v0.13
  ...
  it("debugger Boundaries explicitly forbid Edit / Write / NotebookEdit", async () => {
    const file = path.join(AGENTS_DIR, "debugger.md");
    const body = await fs.readFile(file, "utf8");
  ...
  it("reviewer frontmatter pins model: sonnet (Reason class)", async () => {
    const file = path.join(AGENTS_DIR, "reviewer.md");
    const body = await fs.readFile(file, "utf8");
    expect(body).toMatch(/^model:\s*sonnet\s*$/m);
  });

  it("qa frontmatter pins model: sonnet (Reason class)", async () => {
    const file = path.join(AGENTS_DIR, "qa.md");
    const body = await fs.readFile(file, "utf8");
    expect(body).toMatch(/^model:\s*sonnet\s*$/m);
  });

  it("debugger frontmatter pins model: sonnet (Reason class — multi-step hypothesis)", async () => {
    const file = path.join(AGENTS_DIR, "debugger.md");
    const body = await fs.readFile(file, "utf8");
    expect(body).toMatch(/^model:\s*sonnet\s*$/m);
  });
  ...
  it("architect frontmatter pins model: sonnet (Reason class)", async () => {
    const file = path.join(AGENTS_DIR, "architect.md");
    const body = await fs.readFile(file, "utf8");
    expect(body).toMatch(/^model:\s*sonnet\s*$/m);
  });

  it("dba frontmatter pins model: sonnet (Reason class)", async () => {
    const file = path.join(AGENTS_DIR, "dba.md");
    const body = await fs.readFile(file, "utf8");
    expect(body).toMatch(/^model:\s*sonnet\s*$/m);
  });

  it("security frontmatter pins model: sonnet (Reason class)", async () => {
    const file = path.join(AGENTS_DIR, "security.md");
    const body = await fs.readFile(file, "utf8");
    expect(body).toMatch(/^model:\s*sonnet\s*$/m);
  });
  ...
  // 2026-05 capability reinforcement: developer pinned to Opus.
  ...
  it("developer frontmatter pins model: opus (capability contract)", async () => {
    const file = path.join(AGENTS_DIR, "developer.md");
    const body = await fs.readFile(file, "utf8");
    expect(body).toMatch(/^model:\s*opus\s*$/m);
  });

  // 2026-05 implementer (v0.13): dedicated Phase 8 executor.
  // Pinned to Opus — same rationale as developer (highest-stakes
  // step of the squad). The "no commit/push" + "no AI attribution" + "no
  // scope creep" boundaries are core to the role; pin them by grep.
  it("implementer frontmatter pins model: opus (capability contract)", async () => {
    const file = path.join(AGENTS_DIR, "implementer.md");
    const body = await fs.readFile(file, "utf8");
    expect(body).toMatch(/^model:\s*opus\s*$/m);
  });

  it("implementer Boundaries explicitly forbid git commit / push and AI attribution", async () => {
    const file = path.join(AGENTS_DIR, "implementer.md");
    const body = await fs.readFile(file, "utf8");
  ...
  it("implementer carries the untrusted-input clause", async () => {
    const file = path.join(AGENTS_DIR, "implementer.md");
    const body = await fs.readFile(file, "utf8");
    expect(body).toMatch(/untrusted input/i);
  });
