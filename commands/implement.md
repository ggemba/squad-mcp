...
- `--deep` → force-include `architect` + `security`, allow 3 reject-loop cycles, suggest Codex (still gated on `--codex` consent). Auto-detect picks this on `risk == High` or `work_type == Security` or any of `touches_auth / money / migration`. Example: `/squad:implement --deep refactor src/auth/jwt-validator`.

If the user FORCES `--quick` on a high-risk diff (auth / money / migration), the cap stays at 2 but `security` is force-included as one of the two. The output will carry `mode_warning` — surface that to the user, do not bury it.
